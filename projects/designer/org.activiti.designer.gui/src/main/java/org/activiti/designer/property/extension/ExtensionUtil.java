...
public final class ExtensionUtil {
				...
				if (extensionJars.length > 0) {
					...
					for (final IClasspathEntry classpathEntry : extensionJars) {
						...
						if (classpathEntry.getEntryKind() == 1 && classpathEntry.getContentKind() == 2) {
							...
							// Inspect the jar by scanning its classpath and looking for classes that implement
							// CustomServiceTask
											...
											if (classFile.isClass()) {
												...
												try {
													...
													if (DefaultPaletteCustomizer.class.isAssignableFrom(clazz)) {
														try {
															DefaultPaletteCustomizer DefaultPaletteCustomizer = (DefaultPaletteCustomizer) clazz
																	.newInstance();
															// Add this DefaultPaletteCustomizer to the result
															result.addAll(DefaultPaletteCustomizer
	 ...
	 * @param bo
	 *            the business object
	 * @return true if the business object is a custom service task, false otherwise
	 */
	...
	/**
	 * Gets a list of {@link CustomServiceTask} objects based on the {@link TabbedPropertySheetPage} provided.
	 * 
	 * @param tabbedPropertySheetPage
	 *            the property sheet page linked to a diagram in a project that has {@link CustomServiceTask}s defined
	 * @return a list of all {@link CustomServiceTask}s or an empty list if none were found
	 */
	...
	/**
	 * Gets a list of {@link CustomServiceTask} objects based on the {@link IProject} provided.
	 * 
	 * @param project
	...
	/**
	 * Gets a list of {@link CustomServiceTaskContext} objects based on the {@link IProject} provided.
	 * 
	 * @param project
	 *            the project that has {@link CustomServiceTask}s defined
	 * @return a list containing the context of each {@link CustomServiceTask} object found or an empty list if
	 *         {@link CustomServiceTask}s were found were found
	 */
