...
import org.activiti.designer.common.registry.XmlAdapterRegistry;
...
public abstract class XmlAdapter implements PropertyChangeListener, INodeAdapter {
	...
	protected void addElements(SemanticElement[] activitiElements) {
		for (int i = 0; i < activitiElements.length; i++) {
			addElement(activitiElements[i]);
		}
	}
	...
	protected void removeElement(SemanticElement activitiElement) {
		if (activitiElement == null) return;
		XmlAdapter activitiElementDomAdapter = getFactory().getRegisteredAdapterFor(activitiElement);
		if (activitiElementDomAdapter != null) {
			activitiElementDomAdapter.unregister();
			activitiElement.removePropertyChangeListener(activitiElementDomAdapter);
			activitiElementDomAdapter.setSemanticElement(null);			
		}
		removeElement(activitiElementDomAdapter);
	}
	
	protected void addElement(SemanticElement activitiElement) {
		if (activitiElement == null) return;
		XmlAdapter activitiElementDomAdapter = getFactory().getRegisteredAdapterFor(activitiElement);
		if (activitiElementDomAdapter == null) {
			activitiElementDomAdapter = getFactory().createAdapterFromModel(activitiElement);
			activitiElementDomAdapter.setSemanticElement(activitiElement);
			activitiElement.addPropertyChangeListener(activitiElementDomAdapter);
			activitiElementDomAdapter.register();
		}
		addElement(activitiElementDomAdapter);
	}
	...
	private void unregister() {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			XmlAdapter activitiElementDomAdapter = getAdapter(nodeList.item(i));
			if (activitiElementDomAdapter != null) {
				activitiElementDomAdapter.unregister();
			}
		}		
	...
	public void initialize(SemanticElement activitiElement) {
		setSemanticElement(activitiElement);
		NodeList nodeList = getNode().getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
			XmlAdapter activitiElementDomAdapter = getAdapter(nodeList.item(i));
			if (activitiElementDomAdapter != null && !"#comment".equals(activitiElementDomAdapter.getNode().getNodeName())) {
				doModelAdd(activitiElementDomAdapter);
			}
		}
		register();
	}
