...
public class ActivitiDiagramEditor extends DiagramEditor {
  ...
  public void doSave(final IProgressMonitor monitor) {
    ...
    activeGraphicalViewer = (GraphicalViewer) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor()
    ...
    if (marshallers.size() > 0) {
      ...
      try {
        final ExportMarshallerRunnable runnable = new ExportMarshallerRunnable(diagram, marshallers);
  ...
  protected ContextMenuProvider createContextMenuProvider() {
    return new ActivitiEditorContextMenuProvider(getGraphicalViewer(), getActionRegistry(), getConfigurationProvider());
  }
