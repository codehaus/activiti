...
public class GenericElementXmlAdapter extends XmlAdapter {
	...
	public void initialize(SemanticElement activitiElement) {
		super.initialize(activitiElement);
		if (activitiElement == null || !(activitiElement instanceof GenericElement)) return;
		GenericElement genericElement = (GenericElement)activitiElement;
	...
	protected void doModelAdd(XmlAdapter child) {
		...
		if (getElementType().equals(type)) {
			SemanticElement activitiElement = getSemanticElementFactory().createById("org.activiti.designer.eclipse.genericElement");
			child.initialize(activitiElement);
			SemanticElement parent = getSemanticElement();
			if (parent instanceof GenericElement) {
				GenericElement genericElement= (GenericElement )getSemanticElement();
				genericElement.addGenericElement((GenericElement)activitiElement);
			}
		}
	}
