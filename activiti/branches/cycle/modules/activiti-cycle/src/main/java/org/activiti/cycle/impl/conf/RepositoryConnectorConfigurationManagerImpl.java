 ...
 */
public class RepositoryConnectorConfigurationManagerImpl implements RepositoryConnectorConfigurationManager {

  // TODO: Set a config dir for xstream?
  private static final String CONFIG_DIR = "";
  private static final String FILE_EXT = ".xml";

  private List<Class< ? extends RepositoryConnector>> registeredRepositoryConnnectors = new ArrayList<Class< ? extends RepositoryConnector>>();
  private List<RepositoryConnectorConfiguration> repoConnectorConfigurations = findAllRepositoryConfigurations();

  public RepositoryConnectorConfigurationManagerImpl() {
  }
  ...
  public RepositoryConnectorConfiguration createRepositoryConfiguration(Class< ? extends RepositoryConnector> repositoryConnector, String user,
    ...
    try {
      if (registeredRepositoryConnnectors.contains(repositoryConnector)) {
        ...
        config = (RepositoryConnectorConfiguration) Class.forName(repositoryConnector.getCanonicalName() + "Configuration").newInstance();
        config.setUser(user);
      ...
      throw new RepositoryException("RepositoryConnector '" + repositoryConnector.getClass().getName() + "' is not registered!");
    } catch (Exception e) {
      throw new RepositoryException("Unable to create configuration for " + repositoryConnector.getClass().getName(), e);
    }
  }

  public void registerRepositoryConnector(Class< ? extends RepositoryConnector> repositoryConnector) {
  ...
  public void persistRepositoryConfiguration(RepositoryConnectorConfiguration config) {
    String configFileName = config.getClass().getSimpleName() + FILE_EXT;
    try {
  ...
  public RepositoryConnector createRepositoryConnectorFromConfiguration(RepositoryConnectorConfiguration repositoryConfig) {
    try {
      Class connectorClass = Class.forName(repositoryConfig.getClass().getName().replace("Configuration", ""));
      return (RepositoryConnector) connectorClass.getConstructor(repositoryConfig.getClass()).newInstance(repositoryConfig);
    } catch (Exception e) {
  ...
  public List<RepositoryConnectorConfiguration> findAllRepositoryConfigurations() {
    List<RepositoryConnectorConfiguration> configs = new ArrayList<RepositoryConnectorConfiguration>();

    for (Class< ? extends RepositoryConnector> connector : registeredRepositoryConnnectors) {
      String clazzName = connector.getName() + "Configuration";
      try {
        RepositoryConnectorConfiguration config = (RepositoryConnectorConfiguration) Class.forName(clazzName).newInstance();
        configs.add(findRepositoryConfiguration(config.getClass()));
      } catch (Exception e) {
        throw new RepositoryException("Unable to find class '" + clazzName + "'", e);
      }
    }
  ...
  public RepositoryConnectorConfiguration findRepositoryConfiguration(Class< ? extends RepositoryConnectorConfiguration> config) {
    String configFileName = config.getSimpleName() + FILE_EXT;
    try {
      XStream xStream = new XStream();
      RepositoryConnectorConfiguration loadedConfig = (RepositoryConnectorConfiguration) xStream.fromXML(new FileReader(configFileName));
      return loadedConfig;
    } catch (FileNotFoundException fnfe) {
      throw new RepositoryException("Unable to find RepositoryConnectorConfiguration '" + configFileName + "'", fnfe);
    }
  }
