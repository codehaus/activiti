 ...
 */
public class NotifyingBlockingThreadPoolExecutor extends ThreadPoolExecutor implements IThreadPoolExecutor {

  private static Logger log = Logger.getLogger(NotifyingBlockingThreadPoolExecutor.class.getName());
  ...
  private Condition unpaused = pauseLock.newCondition();
	 ...
	 */
	private AtomicInteger tasksInProcess = new AtomicInteger(0);
	 ...
	 */
	public NotifyingBlockingThreadPoolExecutor(String name, int poolSize, int maxSize, int queueSize, long keepAliveTime, TimeUnit keepAliveTimeUnit, long maxBlockingTime, TimeUnit maxBlockingTimeUnit, Callable<Boolean> blockingTimeCallback) {
	 ...
	 */
	public NotifyingBlockingThreadPoolExecutor(String name, int poolSize, int queueSize, long keepAliveTime, TimeUnit unit) {
  ...
  public String getName() {
	...
	private static class BlockThenRunPolicy implements RejectedExecutionHandler {
		...
		public void rejectedExecution(Runnable task, ThreadPoolExecutor executor) {           
		  ...
		  log.log(Level.FINE, "processing rejection for queue: " + ((NotifyingBlockingThreadPoolExecutor)executor).getName());
				...
				try {
					...
					if(blockingTimeCallback != null) {
						...
						if (workQueue.offer(task, maxBlockingTime, maxBlockingTimeUnit)) {
							// task accepted
						  log.log(Level.FINE, "task accepted in rejection for queue: " + ((NotifyingBlockingThreadPoolExecutor)executor).getName());
							taskSent = true;
						}
						else {
	...
	public void clearQueue() throws Exception {
	  ...
	  if (!isPaused) {
	    throw new Exception("Queue should be paused first and no threads running");
